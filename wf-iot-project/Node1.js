...
const PRIVATE_KEY = process.env.NODE1_PRIVATE_KEY;
const CONTRACT_ADDRESS = process.env.CONTRACT_ADDRESS;
const PRIVATE_SC_ADDRESS = process.env.PRIVATE_CONTRACT_ADDRESS;

const web3 = new Web3('http://192.168.0.13:8545'); // Geth RPC endpoint
...
//const gtwCatalogContractAddress = PRIVATE_SC_ADDRESS; //'0x137720d6580e9f02bD2Fe726884f20F51F7F80ca';
...
async function generateNodeIpnsKey(ipfs, deviceID) {
  ...
  //no caso dos nodos comuns, eles nao tem a chave e precisam importar
  const catalogIpnsKeyName = await importCatalogIpnsKeyName(ipfs);
  await publishToIpns(ipfs, fileAdded.cid, catalogIpnsKeyName, 5000); 
}
...
async function getLastDeviceData(ipfs, fileName) {

  const keys = await ipfs.key.list();
  const key = keys.find((k) => k.name === nodeIpnsKey);
  ...
  const content_string = Buffer.concat(metadata_chunks).toString()
  console.log("IPFS output: ", content_string);
...
async function main() {
    ...
    const filePath = './Nodes/Farm1/Node1/Devices/'
