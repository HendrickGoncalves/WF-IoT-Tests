...
async function ipfsCidTx(ipfs, hash) {
  const currentTimestamp = moment().format('YYYY-MM-DD HH:mm:ss:SSS');

  const cid = await ipfs.add({
    path: "ipfs_file.txt",
    content: currentTimestamp + ';' + hash
  });
...
async function main() {
    ...
    const filePath = './Nodes/Farm1/Node0/Devices/'
